 package com.orbit.app.regression

import androidx.test.ext.junit.runners.AndroidJUnit4
import com.orbit.app.overlay.EdgeSide
import com.orbit.app.overlay.OverlayViewModel
import com.orbit.app.service.ClipboardFocusStateMachine
...
class Spec001SmokeTest {
     ...
     * `app/src/test/java/com/orbit/app/service/ClipboardFocusStateMachineTest.kt`,
    ...
    /**
     * (c) `OrbitOverlayService` MUST schedule a restart so OEM task-killer
     ...
     */
    @Test
    fun orbitOverlayService_exposesScheduleRestartHookForOemKillRecovery() {
        val cls = Class.forName("com.orbit.app.service.OrbitOverlayService")
        val hasScheduleRestart = cls.declaredMethods.any { it.name == "scheduleRestart" }
        assertTrue(
            "OrbitOverlayService.scheduleRestart() must exist (FR-025 OEM kill recovery)",
            hasScheduleRestart
        )
        // RestartReceiver is the AlarmManager target.
        Class.forName("com.orbit.app.service.RestartReceiver")
    }

    /**
     * (d) `OrbitOverlayService.onCreate` MUST reference
     ...
     */
    @Test
    fun orbitOverlayService_canReferenceForegroundServiceStartNotAllowed() {
    ...
    fun actions_do_not_break_capture() {
        // 1. OrbitOverlayService still owns scheduleRestart (FR-025 OEM kill recovery).
        val overlayCls = Class.forName("com.orbit.app.service.OrbitOverlayService")
        assertTrue(
            "OrbitOverlayService.scheduleRestart() must remain after 003",
        ...
        val executorCls = Class.forName("com.orbit.app.action.ActionExecutorService")
        assertTrue(
            "ActionExecutorService must be a Service subclass",
            android.app.Service::class.java.isAssignableFrom(executorCls)
        )
        assertTrue(
            "ActionExecutorService and OrbitOverlayService must be distinct classes",
