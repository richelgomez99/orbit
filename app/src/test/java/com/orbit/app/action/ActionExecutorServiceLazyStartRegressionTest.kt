 ...
 * `bindService(ACTION_BIND_EXECUTOR)`; in particular, `OrbitOverlayService`
...
class ActionExecutorServiceLazyStartRegressionTest {
    ...
    @Test
    fun orbitOverlayService_does_not_eager_start_action_executor() {
        val src = resolve("src/main/java/com/orbit/app/service/OrbitOverlayService.kt").readText()
        assertFalse(
            "OrbitOverlayService must not reference ActionExecutorService — 003 lazy-start invariant.",
            src.contains("ActionExecutorService")
        )
        assertFalse(
            "OrbitOverlayService must not reference BIND_ACTION_EXECUTOR.",
    ...
    fun capture_process_has_no_other_eager_startService_for_executor() {
        ...
        val roots = listOf(
            "src/main/java/com/orbit/app/service",
            "src/main/java/com/orbit/app/overlay",
            "src/main/java/com/orbit/app/action",
        ).map(::resolve)
    ...
    fun action_executor_service_declares_no_default_startCommand_path() {
        ...
        val src = resolve("src/main/java/com/orbit/app/action/ActionExecutorService.kt").readText()
