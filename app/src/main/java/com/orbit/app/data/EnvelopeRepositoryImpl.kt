...
class EnvelopeRepositoryImpl(
        ...
        val job = scope.launch(Dispatchers.IO) {
            backend.observeDayWithResults(isoDate).collectLatest { rows ->
                try {
                    ...
                    val parcel = com.orbit.app.data.ipc.DayPageParcel(
                        isoDate = isoDate,
    ...
    override fun completeUrlHydration(
        ...
        runBlocking {
            ...
            val dedupeExisting: com.orbit.app.data.entity.ContinuationResultEntity? =
                if (ok && !canonicalUrlHash.isNullOrEmpty())
                    backend.findContinuationResultByCanonicalUrlHash(canonicalUrlHash)
                else null

            val result: com.orbit.app.data.entity.ContinuationResultEntity? = if (ok && dedupeExisting == null) {
                com.orbit.app.data.entity.ContinuationResultEntity(
                    id = UUID.randomUUID().toString(),
    ...
    override fun lookupAppFunction(functionId: String): com.orbit.app.data.ipc.AppFunctionSummaryParcel? {
        val delegate = actionsDelegate ?: return null
        return runBlocking { delegate.lookupAppFunction(functionId) }
    }

    override fun listAppFunctions(appPackage: String): List<com.orbit.app.data.ipc.AppFunctionSummaryParcel> {
    ...
    override fun recordActionInvocation(
        ...
        val parsed = runCatching {
            com.orbit.app.data.model.ActionExecutionOutcome.valueOf(outcome)
        }.getOrElse { com.orbit.app.data.model.ActionExecutionOutcome.FAILED }
        runBlocking {
    ...
    override fun stopObservingProposals(observer: com.orbit.app.data.ipc.IActionProposalObserver) {
    ...
    override fun observeClusters(observer: com.orbit.app.data.ipc.IClusterObserver) {
        val key = IBinderKey(observer.asBinder())
        observerJobs.remove(key)?.cancel()
    ...
    override fun stopObservingClusters(observer: com.orbit.app.data.ipc.IClusterObserver) {
        val key = IBinderKey(observer.asBinder())
        observerJobs.remove(key)?.cancel()
    }
