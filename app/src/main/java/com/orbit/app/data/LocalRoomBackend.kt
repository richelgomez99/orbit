...
class LocalRoomBackend(
    ...
    override suspend fun softDeleteTransaction(
        ...
        database.withTransaction {
            invalidateUnderstandingInCurrentTransaction(
                id = id,
                at = deletedAt,
    ...
    private suspend fun invalidateUnderstandingInCurrentTransaction(
        id: String,
        at: Long,
        reason: InvalidationReason
    ) {
        val invalidationDao = database.invalidationRecordDao()
        if (!invalidationDao.existsForCapture(id)) {
            invalidationDao.insert(
        ...
        database.captureUnderstandingDao().markInvalidated(id, at)
        database.evidenceBundleDao().deleteByCapture(id)
        database.activeIntentDao().invalidateByCapture(id, "SOURCE_DELETED", at)
        auditDao.insert(
            AuditLogEntryEntity(
    ...
    override suspend fun insertDigestTransaction(
        envelope: com.orbit.app.data.entity.IntentEnvelopeEntity,
    ...
    override suspend fun listRegularEnvelopesInWindow(
        ...
        limit: Int
    ): List<com.orbit.app.data.entity.IntentEnvelopeEntity> =
    ...
    override suspend fun cascadeDigestInvalidation(
        ...
        now: Long,
        auditFor: (String) -> com.orbit.app.data.entity.AuditLogEntryEntity
    ): List<String> {
    ...
    override suspend fun listRetryableContinuations(
        ...
        continuationDao.getByEnvelopeId(envelopeId)
            .filter { it.status != com.orbit.app.data.model.ContinuationStatus.SUCCEEDED }

    override suspend fun completeUrlHydrationTransaction(
        continuationId: String,
        result: ContinuationResultEntity?,
        newStatus: com.orbit.app.data.model.ContinuationStatus,
        completedAt: Long,
