...
refit. The refit is feature-flagged (`RuntimeFlags.useNewVisualLanguage`), initially
developed behind a default-OFF flag and now default-ON for alpha builds after the
post-Phase-4 flip decision. It shipped behind 5 phases with a Claude review gate after every commit, and
...
- **FR-015-001**: System MUST add a `RuntimeFlags.useNewVisualLanguage: Boolean`
  flag in `app/src/main/java/com/orbit/app/RuntimeFlags.kt`. It was default
  `false` during staged rollout and is now default `true` for the alpha branch
  after T015-904.
...
- The flag default flip from `false` to `true` was approved after Phase 4 and
  is active for the alpha branch.
