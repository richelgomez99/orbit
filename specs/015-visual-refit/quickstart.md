...
- `RuntimeFlags.useNewVisualLanguage` = true in the alpha branch after T015-904;
   older phase-gate checks may override it locally when validating flag-off behavior.
...
- [x] Phase 0 commit 1 begun
- [x] Phase 0 landed (`T015-001` through `T015-020`)
- [ ] Phase 1+ dirty worktree split before PR

As of 2026-04-29 all 3 dependencies are resolved: DEP-001 → spec 016
(separate, parallel), DEP-002 → spec.md confirmed, DEP-003 → PR #3 + PR #4
merged into main before Phase 0 started. Phase 0 has landed; Phase 1+ work
must still be split and reviewed before PR.
