...
- [x] **T015-005** [P0c1] Add `RuntimeFlags.useNewVisualLanguage: Boolean`
  in `RuntimeFlags.kt`, initially default `false`. Add `@Volatile` per existing
...
- [x] **T015-904** Decision point: flip `RuntimeFlags.useNewVisualLanguage`
  default to `true` (or alpha-build override).
  ...
  - 2026-05-18 branch hygiene decision: source default flips to `true` for the
    alpha branch so dogfood installs show Quiet Almanac by default.
