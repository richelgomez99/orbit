...
Open decision for planning: if a capture has multiple URLs, v1 should likely use
the first URL returned by `ContinuationEngine.extractUrls(...)` as the primary
duplicate key and leave multi-URL set matching for a future spec.
