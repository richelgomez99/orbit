...
- Small overlay geometry fixes that preserve existing visuals may land here if
  they are needed for the `Already saved` / undo feedback contract.
...
- **FR-017-005**: `Already saved` feedback MUST offer note, reclassify, and open
  existing capture actions. If note support needs a schema field, it must be
