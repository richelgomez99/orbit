...
- **Next**: `:ml` single-engine inference over AIDL (now a correctness need, per the one-engine finding); confirm classifier quality once single-engine routing lands; `extractActions` via constrained decoding.
