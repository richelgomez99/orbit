...
- **Debug broadcast harness — ROOT CAUSE FOUND**: long-running model-action broadcasts (`DEBUG_TEST_INFERENCE/ROUTED/CLASSIFY`) only fire when **the app is genuinely FOREGROUND** (screen on, activity visible). Samsung suspends/kills background broadcast-receiver coroutines that run more than ~1-2s of work, so these were dying mid-run; `DEBUG_DUMP` always worked because it finishes in <1s. **To test on device: keep the app foreground on-screen while firing** (don't let it doze during the poll). `DebugDumpReceiver` now logs `onReceive`/`coroutine entered` so this is diagnosable.
