...
- **Classifiers** (done, `e3786e5`): `classifyIntent`/`scanSensitivity` are real prompt-and-parse over `generate()`; `extractActions` stays a safe empty default (schema-constrained gen unreliable on 1B). Confidence is heuristic (no token probs from `generateResponse`).
...
- **`:ml` single-engine over AIDL** (code-complete, `2012adf`; **runtime validation pending**): `ILocalInference` AIDL + `LocalInferenceService` (`:ml`, owns the one engine) + `RemoteLocalLlmProvider` (proxy used by every process except `:ml`). Router returns the in-process engine on `:ml`, the proxy elsewhere. Gated behind `localAiEnabled` (default off) so it can't regress the cloud path. On-device validation was blocked by a **system-wide low-memory kill event** (Samsung LMK culled `:ml` + ~12 apps mid-load, aggravated by many engine loads in one session) plus the model-action broadcast flakiness — **reboot the device, then validate**: toggle On-device AI, trigger a real summary/day-header, confirm `creating BYOM engine` logs from the `:ml` pid only (not `:ui`).
...
- **Debug broadcast harness caveat**: long-running model-action debug broadcasts (`DEBUG_TEST_INFERENCE/ROUTED/CLASSIFY`) are unreliable on the S24 Ultra even on a fresh reboot — they frequently don't execute their branch while `DEBUG_DUMP` always does. Root cause undetermined (ruled out memory + foreground). Prefer unit/instrumented tests or the real UI flow for validation over these hooks.
