...
- Spec 021 artifacts were regenerated on 2026-06-14. Scope is a safe internal typed runtime over current `AgentPlanParcel` outputs: document/component model, Compose renderer, and deterministic text fallback. The branch now adds `com.orbit.app.generativeui`, maps existing agent plans into capped typed components, renders the Orbit tab plan result through that runtime, preserves cited evidence opening, and keeps fallback text deterministic. External A2UI dependency, provider JSON parsing, durable chat sessions, direct action execution, and BYOM/local-model-manager work remain deferred. Focused and full non-phone gates passed on 2026-06-14.
- Queue row 13 (`022-local-model-manager`) is active on branch `feature/022-local-model-manager-20260614`.
- Spec 022 artifacts were regenerated on 2026-06-14. Scope is local model policy and router seam first: explicit Speed/Intelligence/legacy Nano/Cloud/Unavailable selection, tier capabilities, hardware profile checks, and JVM tests. Native LiteRT-LM/MLC binaries, downloads, JNI/Vulkan integration, model-store UI, and memory profiling automation remain deferred behind the provider seam.
