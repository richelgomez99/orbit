...
- Spec 008 artifacts were regenerated on 2026-06-12. The first implementation slice adds durable preferences, policy/receipt primitives, compact index receipt metadata, a cloud Ask synthesis gate, a cloud AI routing gate, and three distinct Settings controls. Focused JVM tests, compile, androidTest source compile, lint, assemble, and receipt forbidden-key review passed on 2026-06-12. Remaining work is cloud activity status polish, any final task cleanup, connected/manual validation when a phone is available, and branch closeout.
