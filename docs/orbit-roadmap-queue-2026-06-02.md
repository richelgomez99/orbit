...
- Spec 009 artifacts were regenerated on 2026-06-12. Current implementation adds a local Room v10 KG schema, `GraphBackendAdapter`, `RoomGraphBackendAdapter`, provenance-required fact/relationship writes, source invalidation with surviving-support preservation, source-ready migration/adapter tests, deterministic promoted-memory-to-graph-fact projection, and a compact Binder `getGraphWhyThis` projection. External graph products remain adapter candidates only; adapter evaluation closeout is next.
