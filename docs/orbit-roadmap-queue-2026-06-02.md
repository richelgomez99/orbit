...
- Queue row 6 (`010-agent-coordinator`) is active on branch `feature/010-agent-coordinator-20260612`.
- Spec 010 artifacts were regenerated on 2026-06-12. The branch now has deterministic cited agent planning, a minimal Orbit tab planning panel, and advisory model-assisted display copy behind existing `LlmProviderRouter`/Spec 008 controls. The deterministic plan remains authoritative; model output cannot add uncited steps, unknown evidence, or changed function ids. AppFunctions/Spark interop, A2UI, durable chat sessions, and BYOM local model manager remain deferred.
